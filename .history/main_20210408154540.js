...
function addMatchTileDesk(data) {
  ...
  homeTeamDesk.classList.add("team-desk-home");
  ...
  awayTeamDesk.classList.add("team-desk-away");
  ...
  matchTableDesk.appendChild(matchInfo);
  matchTileDesk.appendChild(matchTimer);
  matchTeamDesk.appendChild(homeTeamDesk);
  matchTeamDesk.appendChild(matchScoreDesk);
  matchTeamDesk.appendChild(awayTeamDesk);
  matchTileDesk.appendChild(matchTeamDesk);
...
function addPremiershipTable(data) {
  ...
  .then((response) =>
    response.json().then((data) => {
      ...
      for (var i = 0; i < leagueTable.length; i++) {
        addPremiershipTable(leagueTable[i]);
      }
    })
  )
  .catch((err) => {
...
function addCountryList(data) {
  var countryListLink = document.createElement("a");
  countryListLink.classList.add("country-option-link");
  countryListLink.href = "#";

  var homeLeft = document.createElement("div");
  homeLeft.classList.add("country-option-link-home");
  homeLeft.innerHTML = "Home";

  var liveLeft = document.createElement("div");
  liveLeft.classList.add("country-option-link-live");
  liveLeft.innerHTML = "Live";

  var countryList = document.createElement("li");
  countryList.classList.add("country-option-link-list");
  countryList.innerHTML = data["name"];

  countryListLink.appendChild(homeLeft);
  countryListLink.appendChild(liveLeft);
  countryListLink.appendChild(countryList);
  countryOption.appendChild(countryListLink);
}
